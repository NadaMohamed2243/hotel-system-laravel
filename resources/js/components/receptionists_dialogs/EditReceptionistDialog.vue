<template>
    ...
    <form @submit.prevent="submitForm" class="space-y-4" v-if="receptionist">
        ...
        <div class="relative">
            <img
            v-if="form.avatarPreview || receptionist.avatar_image"
            :src="form.avatarPreview || receptionist.avatar_image"
...
const submitForm = () => {
  ...
  form.put(route('admin.receptionists.update', receptionist.value.id), {
    data: formData,
    preserveScroll: true,
    onSuccess: () => close(),
    onError: (errors) => {
